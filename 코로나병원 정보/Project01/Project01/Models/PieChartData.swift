...
class PieChartData: ObservableObject {
    ...
    init(data: [Double],names: [String]) {
        ...
        for index in 0..<data.count {
            let slide = SlideData()
            let dataItem = DataItem(name: names[index], value: data[index])
            dataItem.highlighted = false
            slide.data = dataItem
            ...
            let annotationDeltaX = deltaX * (1.2 + padding)
            let annotationDeltaY = deltaY * (1.2 + padding)
            if -90 <= alpha && alpha < 0 {
